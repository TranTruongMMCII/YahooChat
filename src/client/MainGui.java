...
public class MainGui {
	...
	private void initialize() {
		...
		JLabel lblFriendsName = new JLabel("Tài khoản: ");
		...
		btnChat.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (name.equals("") || Client.clientarray == null) {
					Tags.show(frameMainGui, "Tài khoản không tồn tại", false);
					return;
				}
				if (name.equals(nameUser)) {
					Tags.show(frameMainGui,
							"Không thể tự chat với chính bạn", false);
					return;
				}
				...
				Tags.show(frameMainGui, "Tài khoản không tìm thấy.", false);
			}
		});
		...
		btnExit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int result = Tags.show(frameMainGui, "Bạn chắc chứ?", true);
				if (result == 0) {
					try {
						clientNode.exit();
		...
		lblActiveNow = new JLabel("Tài khoản đang hoạt động");
